...
pub fn init_db(args: InitArgs) -> anyhow::Result<()> {
    ...
    if let Some(fsm_metadata) =
        fs::get_metadata(&fsm_dir).context("failed to retrieve metadata for .fsm directory")?
    {
        ...
        for format in db::FORMAT_LIST {
            ...
            let Some(metadata) =
                fs::get_metadata(&db_file).context("io error when checking for db file")?
            else {
                continue;
            };
            ...
            if metadata.is_file() {
                ...
                return Err(anyhow::anyhow!(
                    "a file system item exists with the name of a db file"
                ));
            }
        }
    } else {
        ...
        std::fs::create_dir(&fsm_dir).context("failed to create .fsm directory")?;
    }
    ...
    db::Context::create(db_file, args.format).context("failed to save new db instance")?;
